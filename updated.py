...
class FaceRecognition:
    def __init__(self, images_path, swap_color_channels, capturing_device, display_video_window_dimension,
        ...
        self.email_mapping = self.load_email_mapping()
    ...
    def load_email_mapping(self):
        email_mapping = {}
        with open('email_mapping.csv', mode='r') as file:
            reader = csv.DictReader(file)
            for row in reader:
                email_mapping[row['NAME']] = row['EMAILID']
        return email_mapping

    def send_email(self, name, email, entry=True):
        try:
            # Set up the server
            ...
            sender_email = 'debjanni.123@gmail.com'
            sender_password = 'myns bpht vfjf oocu'  # Replace with your App Password
            server.login(sender_email, sender_password)

            # Create the email content
            subject = f"{name} {'Entry' if entry else 'Exit'} Notification"
            body = f"{name} has {'entered' if entry else 'exited'} the office at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}."
            msg = MIMEText(body)
            msg['Subject'] = subject
            ...
            print(f"Email sent to {email} for {name} - {'Entry' if entry else 'Exit'}")
        
        except Exception as e:
            print(f"Failed to send email to {email} for {name} - {'Entry' if entry else 'Exit'}: {e}")

    def mark_attendance(self, name, filename):
        ...
        email = self.email_mapping.get(filename, None)
        if not email:
        ...
        if name not in name_list:
            ...
            self.send_email(name, email, entry=True)  # Send entry email
        else:
                ...
                if value['name'] == name:
                    ...
                    if (current_time - entry_time).total_seconds() >= 60:
                        if name in self.last_detection and (current_time - self.last_detection[name]).total_seconds() >= 5:
                            ...
                            self.send_email(name, email, entry=False)  # Send exit email
            ...
            while True:
                ...
                for encode_face, face_location in zip(encodes_cur_frame, faces_cur_frame):
                    ...
                    if matches[match_index]:
                        ...
                        self.mark_attendance(name, my_image_list[match_index])
